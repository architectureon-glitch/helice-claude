...
EMBEDDED_SAMPLES = 12  # rayons - sondage d'un niveau de pale pour dire s'il est noye dans une paroi
EMBEDDED_FRACTION = 0.5  # - - part des rayons d'un niveau dans une paroi au-dela de laquelle il est un pied soude
...
DERIVED_MIN_CELLS = 4  # cases - largeur minimale d'un oeillard ou d'une fente pour qu'ils soient lus sur le corps
DERIVED_PROBE_CELLS = 2  # cases - profondeur de la sonde dans le flasque d'extremite, et en retrait du bord
DERIVED_SLICE_THICKNESS = 0.001  # m - epaisseur des tranches fluide deduites du corps
DERIVED_SLICE_GAP = 0.0005  # m - jeu entre une tranche deduite et la paroi qu'elle borde
DERIVED_BORE_RATIO = 0.4  # - - un percement d'extremite plus petit que cette part de l'autre est le passage de l'arbre, pas une entree
BLADES_Z_TOLERANCE = 0.0002  # m - ecart de hauteur au-dela duquel une pale fournie a part n'est plus a sa place
BLADES_Z_TOLERANCE_REL = 0.01  # - - meme ecart, en part de la hauteur de la pale
BLADES_VOLUME_TOLERANCE = 0.02  # - - ecart de volume entre deux pales qui devraient etre copies
BLADES_AZIMUTH_TOLERANCE_DEG = 3.0  # degres - ecart au pas de 360/N entre deux pales fournies
BLADES_AZIMUTH_LEVER = 0.0005  # m - bras minimal du centre d'une pale pour que son azimut ait un sens
